/* md5.c
 * Implements MD5 hashing and HMAC for authentication.
 *
 * This is adapted from the MD5 implementation done by Colin Plumb, and
 * as a result is in the Public Domain.
 *
...
char *MD5_tohex(unsigned char *digest) {
...
char *MD5_HMAC(unsigned char *key, unsigned char *str) {
    static struct MD5Context octx;
    struct MD5Context ictx;
    unsigned char ipad[64], opad[64];
    unsigned int i, keylen, slen;
    unsigned char *keyp;
    ...
    memset(ipad+keylen, (char)0x36, 64 - keylen);
    memset(opad+keylen, (char)0x5c, 64 - keylen);

    MD5Init(&ictx);
    MD5Init(&octx);
    MD5Update(&octx, opad, 64);
    MD5Update(&ictx, ipad, 64);
    MD5Update(&ictx, str, slen);
    MD5Final(&ictx);
    MD5Update(&octx, ictx.digest, 16);
    MD5Final(&octx);
    return(MD5_tohex(octx.digest));
}

char *MD5_hash(unsigned char *str, unsigned int len) {
    return(MD5_tohex(MD5_digest(str, len)));
}
